...
The SafeSurf system is designed as a \textbf{machine learning--based real-time phishing detection system} that integrates a modern web interface, browser extension, and a scalable backend architecture.

The system follows a \textbf{modular and layered design}, consisting of:

\begin{itemize}
    \item \textbf{React-based frontend and Chrome extension} for user interaction
    \item \textbf{FastAPI backend} for request handling and model execution \cite{ref20}
    \item \textbf{Hybrid ensemble machine learning model} for phishing detection \cite{ref18}
\end{itemize}

Unlike traditional systems, SafeSurf incorporates:

\begin{itemize}
    \item \textbf{DNS Guard layer} for deterministic early filtering
    \item \textbf{Asynchronous feature extraction} (111 infrastructure and lexical features)
    \item \textbf{Hybrid ensemble} (VotingClassifier + NLP Text model)
\end{itemize}
...
The SafeSurf system is organized into \textbf{three major layers}:
...
\begin{itemize}
    \item Acts as the \textbf{first deterministic security filter}
    \item Blocks:
    \begin{itemize}
...
\begin{itemize}
    \item Extracts exactly \textbf{111 deep features}:
    \begin{itemize}
...
\begin{itemize}
    \item Developed using \textbf{React.js}
    \item Provides:
    \begin{itemize}
...
\section{Conclusion}
\noindent
The SafeSurf system design formally asserts a \textbf{scalable, highly efficient, and intelligent architecture} for modern phishing detection.
